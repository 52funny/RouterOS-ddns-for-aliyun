...
func setRecordParams(record *alidns.AddDomainRecordRequest, conf *ConfigInfo, recordType string) {
	...
	record.TTL = requests.NewInteger(conf.TTL)
}
...
func AddUpdateAliddns(c *gin.Context) {
	...
	if err != nil {
		log.Printf("TTL query params error: %v", err)
		c.String(http.StatusInternalServerError, "parse ttl query error") // 登录失败返回 "loginerr"
		return
	}
	...
	if len(exsitRecordID) > 0 {
		...
		updateRecord.TTL = requests.NewInteger(conf.TTL)
